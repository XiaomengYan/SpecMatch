% get_m2 is different from get_m in the simulation process.
% get_m2 use sec_starlet_trans to generate 1000 signal.
function M=get_m2(wt)
% var(c0) and var(w1).
N=size(wt,2);
J=size(wt,1);
M=zeros(J,N);
...
for i=1:6
    for j=1:N
        if abs(wt(i,j))/sigma(i,j)>2% sqrt(2logN)
            M(i,j)=1;
        end
    end
end
